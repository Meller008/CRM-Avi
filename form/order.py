...
class Order(QMainWindow):
    ...
    def of_ex_torg12(self,edo, head, article, addres, unite, manager_name, no_pcb, firm, new_address):
        ...
        if firm == 'ип':  # Если документ для ИП, то меняем аоставщика
            text = "ИП Рублёв Александр Александрович, ИНН/КПП: 773013683314, Р/с: 40802810938000063715 в ПАО 'Сбербанк'  г.Москва,Корр. счет: 30101810600000000225" \
    ...
    def of_ex_ttn(self, addres, article, auto, driver, unite, manager_name, new_address):
        ...
        if new_address:
            adr = 'Общество с ограниченной ответственностью "Авидевелопмент-М"(ООО"Авидевелопмент-М") ИНН7703561330\\773101001 121351, г. Москва, вн.тер.г. муниципальный округ Кунцево, ул. Молодогвардейская, д. 61 стр. 20, этаж 1 антресоль, ком. 2 тел. (495)150-45-35 р/с 40702810417030008128 в Филиал "ЦЕНТРАЛЬНЫЙ" Банка ВТБ (ПАО)  г. Москва к/с 30101810145250000411 БИК 044525411'
            adr2 = "121351, г. Москва, ул. Молодогвардейская, д. 61 стр. 20"
            sheet["C9"] = adr
            sheet["E68"] = adr
            sheet["B16"] = adr2
    ...
    def of_ex_score(self, article, unite, bank, firm, new_address):
        ...
        if firm == 'ави':
            if bank == "ВТБ":
                ...
                sheet["A3"] = "Расчетный счет: 40702810617030008128 в Филиал 'ЦЕНТРАЛЬНЫЙ' Банка ВТБ (ПАО)  г. Москва"
                sheet["A4"] = "Корр. счет: 30101810145250000411 БИК банка: 044525411"
            elif bank == "Альфа":
                sheet["A2"] = "ИНН/КПП: 7703561330/773001001"
    ...
    def of_word_act(self, price, new_address):
        ...
        if new_address:
            ...
            adr = """Общество с ограниченной ответственностью "Авидевелопмент-М"(ООО"Авидевелопмент-М") ИНН7703561330\\773001001 123995, г.Москва, Кутузовский проезд д.16 стр.15 тел. (495)150-45-35 р/с 40702810617030008128 в Филиал “ЦЕНТРАЛЬНЫЙ” Банка ВТБ (ПАО)  г. Москва к/с 30101810145250000411 БИК 044525411"""
...
class ImportEDI(QDialog):
    def __init__(self, main):
        ...
        self.login = {"Name": "4607191149998", "Password": "4nT!DvNR92NZK!h"}
